...
const Sidebar = ({ collapsed, onClose }) => {
  ...
  const linkItems = [
    ...
    { to: '/teams', icon: UsersThree, label: 'إدارة الفرق', show: user?.role === 'ADMIN' },
  ...
  return (
    <motion.aside
      ...
      className="relative h-screen sticky top-0 flex flex-col bg-[#0d1117] backdrop-blur-xl border-l border-white/5 overflow-hidden"
    >
          ...
          <AnimatePresence>
            {!collapsed && (
              ...
              >
                ...
                {teamData?.team && (
                  <p className="text-[10px] text-cyan-400/60 m-0 truncate">{teamData.team.name}</p>
                )}
              </motion.div>
            )}
          </AnimatePresence>
