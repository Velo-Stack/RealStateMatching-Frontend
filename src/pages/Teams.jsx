...
import { Users, UserPlus, Buildings, Trash, CaretDown, CaretUp, Crown } from 'phosphor-react';
...
const Teams = () => {
    ...
    const removeMemberMutation = useMutation({
        ...
        onError: (err) => toast.error(err.response?.data?.message || 'فشل إزالة العضو'),
    });

    // Update member role (set as manager)
    const updateRoleMutation = useMutation({
        ...
        onError: (err) => toast.error(err.response?.data?.message || 'فشل تحديث الدور'),
    });
                                        ...
                                        {team.members?.length === 0 ? (
                                            ...
                                            team.members?.map((member) => (
                                                <div key={member.id} className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                                                    <div className="flex items-center gap-2">
                                                        <div className={`h-8 w-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${member.role === 'MANAGER' ? 'bg-gradient-to-br from-amber-500 to-orange-500' : 'bg-gradient-to-br from-emerald-500 to-cyan-500'}`}>
                                                            {member.user?.name?.charAt(0) || '?'}
                                                        </div>
                                                        <div>
                                                            <div className="flex items-center gap-1">
                                                                <p className="text-white text-sm">{member.user?.name}</p>
                                                                {member.role === 'MANAGER' && <Crown size={12} className="text-amber-400" weight="fill" />}
                                                            </div>
                                                            <p className="text-slate-500 text-xs">{member.role === 'MANAGER' ? 'مدير الفريق' : 'عضو'}</p>
                                                        </div>
                                                    </div>
                                                    {/* Admin Actions */}
                                                    {isAdmin && (
                                                        <div className="flex items-center gap-1">
                                                            {member.role !== 'MANAGER' && (
                                                                <motion.button
                                                                    ...
                                                                    whileTap={{ scale: 0.9 }}
                                                                    onClick={() => updateRoleMutation.mutate({ teamId: team.id, memberId: member.user?.id, role: 'MANAGER' })}
                                                            ...
                                                            {member.role === 'MANAGER' && (
                                                                <motion.button
                                                                    whileHover={{ scale: 1.1 }}
                                                                    whileTap={{ scale: 0.9 }}
                                                                    onClick={() => updateRoleMutation.mutate({ teamId: team.id, memberId: member.user?.id, role: 'MEMBER' })}
                                                                    disabled={updateRoleMutation.isPending}
                                                            ...
                                                            <motion.button
                                                                ...
                                                                onClick={() => {
                                                                    if (confirm(`هل تريد إزالة ${member.user?.name} من الفريق؟`)) {
                                                                        removeMemberMutation.mutate({ teamId: team.id, memberId: member.user?.id });
                                                                    }
                                                                }}
            ...
            <Modal isOpen={isMemberModalOpen} onClose={() => setIsMemberModalOpen(false)} title={`إضافة عضو إلى ${selectedTeam?.name || ''}`}>
                <form onSubmit={handleAddMember} className="space-y-5 text-right">
                    ...
                    <div>
                        ...
                        {/* تحذير إذا الفريق له مدير بالفعل */}
                        {selectedTeam?.members?.some(m => m.role === 'MANAGER') && memberData.role === 'MANAGER' && (
                        ...
                        <div className="flex gap-2">
                            <motion.button
                                ...
                                className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-medium transition-all ${memberData.role === 'MEMBER'
                            ...
                            <motion.button
                                ...
                                className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-medium transition-all ${memberData.role === 'MANAGER'
