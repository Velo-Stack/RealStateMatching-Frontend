...
/**
 * Live preview of the public project details page for the "إضافة / تعديل مشروع" CMS editor.
 * - Renders the real project page inside an iframe (?cmsPreview=1) using a sentinel
 *   slug, so it works even before the project has been saved (new project flow).
 * - Streams the in-progress (unsaved) project form into the iframe so edits show up
 *   instantly, without needing to click "حفظ".
 * - Auto-scrolls the iframe to whichever accordion section the editor currently has open.
 */
const ProjectLivePreview = ({ payload, anchor }) => {
  ...
  const previewUrl = useMemo(() => {
    ...
    return `${window.location.origin}/projects/__cms_preview__?cmsPreview=1`;
  }, []);
    ...
    <>
      ...
      >
        ...
        <div className="border-t border-white/10 px-4 py-2 text-[11px] text-slate-500">
          كل تعديل تكتبه يظهر هنا فورًا قبل الحفظ — وعند فتح أي قسم من الأقسام جنب، تنتقل المعاينة له تلقائياً.
        </div>
      </div>
    </>
  );
