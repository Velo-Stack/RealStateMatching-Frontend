...
const OfficeFormModal = ({ isOpen, onClose, form, setForm, onSubmit, isEditing, isPending }) => (
    ...
    <form onSubmit={onSubmit} className="space-y-4 text-right">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input className={inputClasses} dir="ltr" value={form.phone} onChange={(e) => setForm((p) => ({ ...p, phone: e.target.value }))} />
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          <label className={labelClasses}>معرف المدينة</label>
          <input type="number" className={inputClasses} dir="ltr" value={form.cityId} onChange={(e) => setForm((p) => ({ ...p, cityId: e.target.value }))} />
        </div>
        <div>
          <label className={labelClasses}>معرف المدير</label>
          <input type="number" className={inputClasses} dir="ltr" value={form.managerUserId} onChange={(e) => setForm((p) => ({ ...p, managerUserId: e.target.value }))} />
        </div>
        <div>
          <label className={labelClasses}>معرف الفريق</label>
          <input type="number" className={inputClasses} dir="ltr" value={form.teamId} onChange={(e) => setForm((p) => ({ ...p, teamId: e.target.value }))} />
        </div>
      </div>
      {isEditing ? (
